...
export default function EditJasaPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();

  const [jasa, setJasa] = useState<ServiceItem | null>(null);
  ...
  useEffect(() => {
    Promise.all([getJasaById(id), getCategories(), getBusinesses()]).then(([data, cats, bizs]) => {
      ...
      setForm({
        ...
        maximum_price: data.maximum_price != null ? String(data.maximum_price) : '',
  ...
  const set = (name: keyof FormState, value: string) => {
    setForm((p) => {
      ...
      if (name === 'service_name' && !jasa?.slug) {
  ...
  if (notFound) {
    return (
      <div className={styles.notFound}>
        ...
        <p className={styles.notFoundTitle}>Layanan jasa tidak ditemukan</p>
        <button className={styles.btnSecondary} onClick={() => router.push('/admin/jasa')}>
          ← Kembali
        </button>
      </div>
    );
    ...
    <form className={styles.page} onSubmit={handleSubmit} noValidate>
      ...
      <div className={styles.adminHeader}>
        <div>
          ...
          <h1 className={styles.title}>Edit {jasa?.service_name}</h1>
          <div className={styles.pbiNote}>Sesuaikan detail layanan jasa UMKM.</div>
        </div>
